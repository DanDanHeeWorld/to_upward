...
value_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
...
contents:[
SK하이닉스, 동종업계 대비 적절한 PER로 투자 매력 유지
...
"""
volitality_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
class:[
...
"""
performance_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
class:[
...
"""
finance_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
class:[
...
"""
business_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
class:[
...
"""
dividend_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
class:[
...
contents:[포스코퓨처엠은 최하위 배당주로 분류됩니다. 1년 평균 배당수익률은 0.07%로, 이는 5년 평균 배당수익률인 0.408%에 비해 크게 낮습니다. 포스코퓨처엠은 포스코그룹의 화학소재 계열사로, 전기차 배터리, 반도체, 디스플레이 등 첨단산업 소재를 생산합니다. 최근 에너지 부문의 성장이 두드러지고 있지만, 여전히 배당 성향은 낮은 수준입니다.
...
"""
growth_str = """주어진 Data만을 활용해서 contents를 작성하는 task이다. 외부의 데이터를 활용해서는 안된다. 글자수 제한은 최대 500자. 각각의 contents에서는 개별기업에 대해서만 다루어야한다.
기존에 작성된 부분은 그대로 두고 bard가 작성해야할 부분에 대해서만 출력해야한다.
...
sector_list = ["dividend", "growth", "value", "business", "finance", "performance", "volitality"]
...
def on_input_change():
    ...
    if st.session_state['chat_stage'] == 1:
        ...
        if user_input not in data['Name'].values:
            st.session_state['generated'].append({"type": "normal", "data": "코스피 200 종목으로 다시 입력해주세요."})
            return
        else:
        ...
        if input_sector not in sector_list:
        ...
        contents:[bard가 작성해야할 부분]""" # 앞의 예시와 붙이기 전의 글
        ...
        if user_input.lower() in ["네", "예", "yes"]:
            ...
            st.session_state['generated'].append({"type": "normal", "data": "챗봇과 자유로운 질문을 하시겠습니까?"})
...
with st.container():
    st.text_input("챗봇과 대화하기:", on_change=on_input_change, key="user_input", help="Clear message 버튼을 누르면 초기화면으로 돌아옵니다.")
...
# 구글 로그아웃&로그인 -> Bard 접속 -> F11(개발자모드) # __Secure-1PSID의 값을 복사하기

# 7가지 섹터, 선택 종목이 목록에 없을 때 오류가 발생하지 않도록 조건문 추가
# chat stage 추가하여 상호작용하기