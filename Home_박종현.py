...
if 'selected_sectors' not in st.session_state:
    st.session_state.selected_sectors = st.multiselect("중요하게 여기는 가치 2가지~3가지를 선택하세요.", options=sectors,max_selections=3, placeholder="섹터를 선택해주세요.")
else:
    st.session_state.selected_sectors = st.multiselect("중요하게 여기는 가치 2가지~3가지를 선택하세요.", options=sectors, max_selections=3,default=st.session_state.selected_sectors, placeholder="섹터를 선택해주세요.")
...
if st.sidebar.checkbox('주식 섹터에 대한 설명 보기', help='클릭 시 설명을 확인할 수 있습니다.', key="sector_info"):
...
if st.sidebar.checkbox('주식 섹터 기준에 대한 설명 보기', help='클릭 시 설명을 확인할 수 있습니다.', key="sector_info2"):
    st.session_state.show_description = True
else:
    st.session_state.show_description = False
...
# "주식 섹터에 대한 설명"이 True면 내용을 표시
if st.session_state.show_description:
    st.write("### 주식 섹터 기준에 대한 설명")

    st.write("#### 배당 (Dividend): ['A', 'B', 'C']")
    st.write("""
    ...
    D (하위 배당주): 이 주식은 배당률이 상대적으로 낮지만, 성장 포텐셜이 있는 주식을 나타냅니다. 높은 성장률을 추구하는 투자자들에게 좋은 선택이 될 수 있습니다. \n
    E (최하위 배당주): 배당률이 낮지만, 높은 성장 잠재력을 가진 주식을 표시합니다. 성장주 투자자들에게 이상적인 선택이 될 수 있습니다. \n
    """)

    st.write("#### 성장 (Growth): ['A', 'A-', 'B', 'B-']")
    st.write("""
    ...
    C (보통의 경영 성과): 이 등급은 일정한 영업이익률과 순이익률을 보여주는 기업을 나타냅니다. 안정적인 경영을 통해 지속 가능한 성장을 추구하는 기업들이 있습니다. \n
    D (저조한 경영 성과): 이 등급은 영업이익률과 순이익률이 상대적으로 낮지만, 성장 포텐셜이 있는 기업을 나타냅니다. 향후 성장 가능성이 있는 기업들이 포함되어 있습니다. \n
    """)    

    st.write("#### 재무 (Finance): ['A+', 'A', 'B', 'C', 'D', 'E']")
    st.write("""
    ...
    st.write("#### 실적 (Performance): ['A+', 'A', 'B', 'C']")
    st.write("""
    ...
    st.write("#### 변동성 (Volatility): ['A+', 'A', 'B', 'C']")
    st.write("""
